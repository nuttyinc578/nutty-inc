...
### Option A: run from a cloned repo

```bash
# From the repository root
sudo bash oneos/build-oneos.sh
```

### Option B: run directly via curl (no checkout required)

```bash
curl -fsSL https://raw.githubusercontent.com/<your-org>/<your-repo>/HEAD/oneos/build-oneos.sh | sudo bash
```
